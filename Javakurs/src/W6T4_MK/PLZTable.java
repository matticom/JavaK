...
public class PLZTable extends JDialog implements ActionListener, ListSelectionListener, KeyListener, MouseListener
{
	...
	private void detailFormat(long PrimaryKey)
	{
		...
		// Neuer Wert einfügen über Menu
		if (PrimaryKey == -1)
		{
			PLZForm dlg = new PLZForm((long)Tabelle.getValueAt(Tabelle.getRowCount()-1, 0)+1);
			dlg.showDialog(this);
		}
		else // ein Wert soll geändert werden
		{
			PLZForm dlg = new PLZForm(PrimaryKey, Tabelle.getValueAt(Tabelle.getSelectedRow(),1).toString(), 
									Tabelle.getValueAt(Tabelle.getSelectedRow(),2).toString());
			dlg.showDialog(this);
		}
