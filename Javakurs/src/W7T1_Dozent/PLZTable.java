...
public class PLZTable extends JDialog implements ActionListener, ListSelectionListener, KeyListener, MouseListener
{
	...
	// Aktualisierung des Datenmodells nicht als Thread
...
//		t.run();
	...
	private void detailFormat(long PrimaryKey)
	{
		...
		else
		{
			...
			{
				showData();
				selectRowByValue("PRIMARYKEY", dlg.getPrimaryKey());
	...
	private int selectRowByValue(String columnName, Object value)
	{
		
		int retValue = ((PLZTableModel)Tabelle.getModel()).findEntry(columnName, value);
		setSelectedRow(retValue);
		return retValue;
	...
	private void updateTableRow(int rowIndex)
	{
		...
		SQL += " WHERE PRIMARYKEY = " + Tabelle.getValueAt(rowIndex, 0).toString();
	...
	private class PLZTableModel extends AbstractTableModel
	{
		...
		public int findEntry(String columnName, Object value)
		{
