...
//			textArea.setSelectionEnd(13);
	...
	public void actionPerformed(ActionEvent e)
	{
		if (e.getSource() == btnWeitersuchen)
		{
			...
			if (findNextEntry() >= 0)
			{
				nextFund = findNextEntry();
				textArea.select(nextFund, nextFund + tfSuche.getText().length());
				//textArea.requestFocusInWindow();
			}
			else 
			{
	...
	private boolean queryNewSearch()
	{
		...
		int dlgValue = JOptionPane.showOptionDialog(this, "Es wurden keine weiteren Einträge gefunden werden, soll das "
															+ "Dokument von Beginn an durchsucht werden?", "Kein weiterer Eintrag", 
	...
	private int findNextEntry()
	{
		...
		{
			suchtxt = tfSuche.getText();
			txt = textArea.getText();
		}
		else
		{
			suchtxt = tfSuche.getText().toLowerCase();
			txt = textArea.getText().toLowerCase();
		}
				
		int start = textArea.getSelectionEnd();
		
		return txt.indexOf(suchtxt, start+1);
	}
