...
public class Datum implements Comparable<Datum>
{
	...
	protected int convertDateToNumber()
	{
		
		// Erstellt eine Ganzzahl für das Datum im Format JJJJMMTT
		return (jahr * 1000) + (monat * 100) + tag;
	...
	public int compareTo(Datum that)
	{
		...
		int retValue = Integer.compare(this.convertDateToNumber(), that.convertDateToNumber());
		
		// Wenn das Datum identisch und das Argument vom Typ 'DatumZeit' ist,
		// den Zeitanteil ebenfalls für den Vergleich verwenden.
		// Für das Datum wird die Zeit 00:00:00 angenomen.
		if (retValue == 0 && that instanceof DatumZeit)
			retValue = Integer.compare(0, ((DatumZeit) that).convertTimeToNumber());
	...
	public boolean equals(Object that)
	{
		...
		if (that instanceof DatumZeit)
			retValue = this.convertDateToNumber() == ((DatumZeit) that).convertDateToNumber() && ((DatumZeit) that).convertTimeToNumber() == 0;
		else if (that instanceof Datum)
				retValue = this.convertDateToNumber() == ((Datum) that).convertDateToNumber();
	...
	public int hashCode()
	{
		...
		//return this.convertDateToNumer();	}
		// oder
		return Integer.hashCode(this.convertDateToNumber());
