...
public class Datenbankzugriffe extends JFrame implements WindowListener, ActionListener
{
	...
	private Fortschrittsanzeige progress;
	...
	private void initializeComponents()
	{
		...
		// Fortschrittsanzeige ganz rechts innerhalb der Statusbar hinzufügen
	...
	{
		Thread tp = new Thread(new InstanceOfProgress());
		tp.start();
	}
	
	private class InstanceOfProgress implements Runnable
	{
		...
		public void run()
		{
			...
			progress.showDialog(Datenbankzugriffe.this);
	...
	private void readFile(String Dateiname)
	{
		...
		{
			max = getRecordCount(Dateiname);
			fileLength = -1;
		}
		progress = new Fortschrittsanzeige("Daten in Datenbank einlesen", max);
		progressDialog();
		...
		try (Scanner scanner = new Scanner(new FileInputStream(Dateiname)))
		{
			...
			while(scanner.hasNextLine() && !progress.isAbort())
			{
//				if (progress.isAbort())
//					System.out.println("Abbruch gedrückt!");
				
				zeile = scanner.nextLine();
				readCounter++;
				if (readCounter % 10 == 0)
					progress.setLabelText(String.format("Datensätze werden gelesen...    [%s]", NumberFormat.getInstance().format(readCounter)));
				
				
				if (fileLength < 0)
					progress.setProgressValue(readCounter);
				else
					progress.setProgressValue(progress.getProgressValue() + zeile.length() + System.lineSeparator().length());
